...
def showSignUp():
    ...
    for opID in lst_key_sql_L3:
        ...
        keyL3 = '''
                SELECT header.testheaderid, header.SERIALNUMBER,header.PARTNUMBER,header.OPERATION,header.OPERATIONID,header.TESTSTATION,header.OPERATORID,header.RESULT
                FROM testheader_v    header  where header.operationid = '344' ORDER BY header.STARTTIME
                '''.format(opID)
        cursorL3.execute(txt_query_L2)
        dataL3 = cursorL3.fetchall()
        cursorL3.close()
        connectionL3.close()
        ...
        for item in dataL3:
            ...
            if key in count_dict:
                ...
                count_dictL3[key] = 1
        # print (count_dict) # MODEL : many of MODEL
        updated_dataL3 = [item + (count_dictL3[item[4]],) for item in dataL3 if item[4] == opID]
