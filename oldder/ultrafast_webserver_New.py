...
def main():
    ...
    FROM testheader_v header WHERE header.productfamilyname ='PicoBlade3' AND header.starttime >= SYSDATE - INTERVAL '5' DAY
        '''
...
def showSignUp():
    ...
    count_dict = {}
    for item in data:
        key = item[2]  # Use the operation name as the key for counting duplicates
        print(key)
    ...
    updated_data = [item + (count_dict[item[2]],) for item in data]
    ...
    minute_time = ""; lst_time = []
    line_count = len(data)
    for item in data:
        start_time = item[6]; end_time = item[7]
        minute_time = int((end_time - start_time).total_seconds() / 60)
        print(minute_time)
        Pump_Tray_Assembly = "{} minutes ... ATTEMPTS OPERTION_ID: {} - {}".format(minute_time, item[4], item[3])
        lst_time.append(Pump_Tray_Assembly)
    print(lst_time)
    ...
    return render_template('signup_new_03.html',all_header_per_sn_list = all_header_per_sn_list
            , serial_num = ser_num
            , model_to_print = key
