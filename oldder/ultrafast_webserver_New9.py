...
def showSignUp():
    ...
    for i in deduplicated.values():
        if PartNumber_item == "":
            PartNumber_item = i[2]
        PartNumber.append(PartNumber_item)
    ...
    #     operationID = i[4]
    #     # print(operationID)
    #     dataL3.append(operationID)
    #     # print(i)

    key_sql_L3 = {item[4]: item for item in updated_data}; lst_key_sql_L3 = []
    for i in key_sql_L3.values():
        operationID = i[4]
        # print(operationID)
        lst_key_sql_L3.append(operationID)
        # print(i)
    # print(lst_key_sql_L3)


    #find SERIAL_NUMBER DUT in to list
    # ------------------------------------------------------------------------------------------
    lst_sernum_L3 = []; lst_opID = []; lst_serItem = []
    for opID in lst_key_sql_L3:
        ...
        for item in data:
            ...
            if key in count_dict:
                ...
                count_dictL3[key] = 1
        # print (count_dict) # MODEL : many of MODEL
        updated_dataL3 = [item + (count_dictL3[item[4]],) for item in data if item[4] == opID]
    ...
    # print(lst_opID)
    # for i in lst_sernum_L3:
    #     print(i)

    # -----------------------------------------------------------------------------------------------------------
    # L3 after click green botton
    last_serItem = lst_sernum_L3[-1]
    print("last_serItem = {}".format(last_serItem))
    lst_stepName = [] 
    # for IDL4 in last_serItem:
    for IDL4 in lst_sernum_L3: #To show all TestStepName delete comment.
        # print(IDL4)
        connectionL4 = cx_Oracle.connect(username, password, dsn)
        cursorL4 = connectionL4.cursor()
        keyL3 = '''
                select step.testheaderid, step.testheaderstepid, step.serialnumber,step.operationname,step.steptype,step.operationstepname,step.result from testheaderstep_v   step
                where step.testheaderid = {0}
                '''.format(IDL4)
        cursorL4.execute(keyL3)
        dataL4 = cursorL4.fetchall()
        cursorL4.close()
        connectionL4.close()

        # Count duplicates and append count as the last index
        #find how many of stepName and find stempName
        num_data = 0
        for item in dataL4:
            lst_stepName.append(item)
            # print(item)
            num_data += 1
        # print("num_data {} = {}".format(IDL4, num_data))
        ...
        for item in dataL4:
            ...
            if key in count_dict:
                ...
                count_dictL4[key] = 1
        # print (count_dict) # MODEL : many of MODEL
        updated_dataL4 = [item + (count_dictL4[item[4]],) for item in data if item[4] == IDL4]
        # for item in updated_dataL4:
        #     print(item)

    num_lst_stepName = 0
    for i in lst_stepName:
        # print(i)
        num_lst_stepName += 1
    # print("num lst stName = {}".format(num_lst_stepName))

    lst_stepID = []
    for i in lst_stepName:
        # print(i[1])
        lst_stepID.append(i[1])
    # -----------------------------------------------------------------------------------------------------------
    # L4 after click Step name

    lst_all = []; lst_numeric = []; lst_string = []; lst_image = []
    for L4 in lst_stepID:
        # print(L4)
        #numeric-----------------------------------------------
        connection_L_numeric = cx_Oracle.connect(username, password, dsn)
        cursor_L_numeric = connection_L_numeric.cursor()
        numeric = '''
                select param.testheaderstepid,param.parametername,param.steptype,param.valuemean, param.units,param.specmin,param.specmax,param.status,param.compoperator from measurementparameter_v   param
                where param.testheaderstepid = {0}
                '''.format(L4)
        cursor_L_numeric.execute(numeric)
        data_L_numeric = cursor_L_numeric.fetchall()
        cursor_L_numeric.close()
        connection_L_numeric.close()

        num_numeric = 0
        for item in data_L_numeric:
            lst_numeric.append(item)
            print(item)
            num_numeric += 1
        # print("num_numeric {} = {}".format(L4, num_numeric))

        #string-----------------------------------------------

        connection_L_str = cx_Oracle.connect(username, password, dsn)
        cursor_L_str = connection_L_str.cursor()
        str = '''
                    select str_param.testheaderstepid,str_param.parametername,str_param.steptype, str_param.operationstepname, str_param.valuestring,str_param.result, str_param.units,str_param.status from stringmeasurement_v   str_param
                    where str_param.testheaderstepid = {0}
                '''.format(L4)
        cursor_L_str.execute(str)
        data_L_str = cursor_L_str.fetchall()
        cursor_L_str.close()
        connection_L_str.close()

        num_str = 0
        for item in data_L_str:
            lst_string.append(item)
            print(item)
            num_str += 1
        # print("num_str {} = {}".format(L4, num_str))

        #Image-----------------------------------------------

        connection_L_image = cx_Oracle.connect(username, password, dsn)
        cursor_L_image = connection_L_image.cursor()

        image = '''
                select image_param.testheaderstepid,image_param.imageid, image_param.imagename, image_param.filenamelong, image_param.operationstepname, image_param.status from imagedata_v   image_param
                where image_param.testheaderstepid = {0}
                '''.format(L4)
        cursor_L_image.execute(image)
        data_L_image = cursor_L_image.fetchall()
        cursor_L_image.close()
        connection_L_image.close()

        num_image = 0
        for item in data_L_image:
            lst_image.append(item)
            print(item)
            num_image += 1
        # print("num_image {} = {}".format(L4, num_image))
    ...
    return render_template('signup_new_08.html',all_header_per_sn_list = all_header_per_sn_list
            ...
            , lst_serItem = lst_serItem
            , lst_stepName = lst_stepName, num_lst_stepName = num_lst_stepName
            ########L4
            , lst_numeric = lst_numeric, lst_string = lst_string, lst_image = lst_image
            , lst_all = lst_all
            )
