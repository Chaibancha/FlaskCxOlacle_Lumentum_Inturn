...
def showSignUp():
    ...
    for opID in lst_key_sql_L3:
        ...
        for item in updated_dataL3:
            ser_num_L3 = item[0]
            # print(ser_num_L3)
            # print("lst_serItem append = {}".format(item))
    ...
    #     print("num_last_serItems = {}".format(i))
    # for item in updated_dataL3:
    last_serItem = []
    for item in last_serItemData:
    ...
    for IDL4 in last_serItem:
        ...
        # Count duplicates and append count as the last index
        # find how many of stepName and find stepName
        num_data = 0
        for item in dataL4:
    ...
    for L4 in lst_stepID:
        ...
        lst_numeric.extend(cursor.fetchall())

        # String
        string_query = '''
            select str_param.testheaderstepid, str_param.parametername, str_param.steptype, str_param.operationstepname, str_param.valuestring, str_param.result, str_param.units, str_param.status
            from stringmeasurement_v str_param
            where str_param.testheaderstepid = :L4
        '''
        cursor.execute(string_query, L4=L4)
        lst_string.extend(cursor.fetchall())
        ...
        lst_image.extend(cursor.fetchall())

    cursor.close()
    connection.close()

    lst_all.extend(lst_numeric)
    lst_all.extend(lst_string)
    lst_all.extend(lst_image)
