...
def showSignUp():
    ...
    # sorted_data = sorted(updated_data, key=lambda x: op_order.index(x[4]))
    # for item in sorted_data:
    #     print(item)
    # print("deduplicated0")
    # deduplicated0 = {item[4]: item for item in sorted_data}
    # for k ,v in deduplicated0.items():
    #     print(k, v)
    ...
    percentage_passed = (num_of_passed / float(num)) * 100
    percentage_passed_rounded = round(percentage_passed, 2)
    print("Percentage of Passed: {}%".format(percentage_passed_rounded))
    ...
    return render_template('signup_new_10.html',all_header_per_sn_list = all_header_per_sn_list
            , serial_num = ser_num, num_attempts = num_attempts, percentage_passed_rounded = percentage_passed_rounded
