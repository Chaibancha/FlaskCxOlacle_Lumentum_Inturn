...
try:
    ...
    try:
        cur = conn.cursor()
        sql = """
            SELECT header.SERIALNUMBER <= 23000000,header.PARTNUMBER,header.OPERATION,header.OPERATIONID,header.TESTSTATION,header.STARTTIME,header.RESULT,header.OPERATORID,SYSTIMESTAMP - header.STARTTIME
            FROM testheader_v header WHERE header.productfamilyname ='PicoBlade3' AND header.starttime >= SYSDATE - INTERVAL '14' DAY
            """
        cur.execute(sql, {'SERIALNUMBER': 22170520})
        rows = cur.fetchall()
        print(rows)
